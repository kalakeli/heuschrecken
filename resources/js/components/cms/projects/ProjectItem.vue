            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        <router-link to="/cms/projekte/liste">
                            <button class="btn btn-sm btn-primary float-end ms-3" title="Projektliste anzeigen">
                        ...
                        <form @submit.prevent="onSave" @keydown="form.onKeydown($event)" >
                            ...
                            <div class="row">
                                <div class="col-lg-9 col-md-9 col-sm-8 col-12">
                                    <div class="form-group mb-3">
                                        ...
                                        <input v-model="form.projectTitle"
                                                name="projectTitle"
                                                type="text"
                                                @blur="onBlur"
                                                class="form-control"
                                                >
                                        <div class="form-error" v-if="form.errors.has('projectTitle')" v-html="form.errors.get('projectTitle')" />
                                    </div>
                                    <hr>
                                    <div class="form-group mb-3">
                                        <label for="Title"><small>URL <sup class="icon mandatory"></sup></small></label>
                                        <input v-model="form.projectUrl"
                                                name="projectUrl"
                                                type="text"
                                                class="form-control"
                                                >
                                        <div class="form-error" v-if="form.errors.has('projectUrl')" v-html="form.errors.get('projectUrl')" />
                                    </div>
                                    <hr>
                                    <div class="form-group mb-3">
                                        ...
                                        <input v-model="form.projectTeaser"
                                                name="projectTeaser"
                                                type="text"
                                                class="form-control"
                                                >
                                        <div class="form-error" v-if="form.errors.has('projectTeaser')" v-html="form.errors.get('projectTeaser')" />
                                    </div>
                                    <hr>
                                </div>
                                <!-- Bild zur Kontrolle -->
                            ...
                            <div class="row">
                                <div class="col-12">
                                    ...
                                    <html-editor v-model="form.projectText" :class="{ 'is-invalid': form.errors.has('projectText') }" name="projectText" id="projectText"></html-editor>
                                </div>
                            </div>
                                ...
                                <div class="col-12">
                                    ...
                                    <div class="row">
                                        <div class="col-6 border-end"> 
                                            <label for="Datum"><small>Datum Veröffentlichung <sup class="icon mandatory"></sup></small></label>
                                        ...
                                        <div class="col-6">
                                                ...
                                                <div class="form-check">
                                                    ...
                                                    <label class="form-check-label" for="flagActive1">
                                                        <small>Projekt wird angezeigt</small>
                                                    </label>
                                                </div>
                                                <div class="form-check">
                                                    ...
                                                    <label class="form-check-label" for="flagActive2">
                                                       <small> Projekt wird <strong>nicht</strong> angezeigt</small>
                                                    </label>
                                                </div>
...
<script>
  ...
  export default defineComponent({
        ...
        methods: {
            ...
            getProject() {
                let self=this;
                axios.get(`${API_URL}/api/projects/${self.theID}`)
                    .then(function(response) {
                        if (response.data)
                        {
                            self.form.dateOfPublication = response.data.dateOfPublication; 
                            self.form.flagActive = response.data.flagActive; 
                            self.form.projectPKID = response.data.projectPKID; 
                            self.form.projectTitle = response.data.projectTitle; 
                            self.form.projectText = response.data.projectText;
                            self.form.projectTeaser = response.data.projectTeaser;
                            self.form.projectUrl = response.data.projectUrl;
                            ...
                            self.emitter.emit('projecttext-loaded', self.form.projectText);
                        }
                    })
                    .catch(() => {
                        PersistantToast.fire({
            ...
            onBlur(event) {
                ...
                axios.post(`${API_URL}/api/projects/friendlyurl`, { params: {
                   url: this.form.projectTitle
                }})  
                .then(({ data }) => {
                    this.form.projectUrl = data;
                });
            },

            onSave() {
                if (this.form.projectPKID>0) 
                {
                    axios.put(`${API_URL}/api/projects/${this.form.projectPKID}`, { params: {
                            id: this.form.projectPKID, 
                            form: this.form
                        }})  
                        .then(function(response) {
                            if ((response.data))
                            {
                                Toast.fire({
                                    icon: 'success',
                                    title: 'Das Projekt wurde aktualisiert'
                            })                            
                            }
                        })
                        .catch(() => {
                            PersistantToast.fire({
                                icon: 'error',
                                title: 'Das Projekt konnte nicht aktualisiert werden!'
                            })
                        });  
                }
                else 
                {
                    this.form.post(`${API_URL}/api/projects`)
                        .then(function(response) {
                            if ((response.data))
                            {
                                Toast.fire({
                                    icon: 'success',
                                    title: 'Das Projekt wurde gespeichert'
                            })                            
                            }
                        })
                        .catch(() => {
                            PersistantToast.fire({
                                icon: 'error',
                                title: 'Das Projekt konnte nicht gespeichert werden!'
                            })
                        });                  
                }
            }, 
