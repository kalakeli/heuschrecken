            ...
            <div class="row">
                <div class="col-12">
                    <form class="row gy-2 gx-3 mb-3 align-items-center">
                        <div class="col-auto">
                            <h4 class="mb-3">
                                <label for="autoSizingInput" class="float-start me-2">Letzte</label>
                                ...
                                <label for="autoSizingInput" class="float-start">Funde</label>
                            </h4>
                        </div>
                    </form>
                </div>
                                            ...
                                            <a :href="`https://daten.heuschrecken-portal.de/kartierliste/${item.id}`" target="_blank">
                                                <div class="d-grid gap-2">
                                                    <button class="btn btn-sm btn-outline-dark text-start mb-2" @mouseover="highlightedItem=item" @mouseout="highlightedItem=null">
                                                        ...
                                                        <!-- <span v-html="dayjs(item.sample.date.from).locale('de').format('DD.MM.YYYY')"></span> | -->
                                                        <span v-html="new Intl.DateTimeFormat('de-DE').format(new Date(item.sample.date.from))"></span> |
                                                        <em>
                                                            <span v-html="item.taxon.name"></span>
                                                        </em>
                                                    </button>                                        
                                                </div>
                                            </a>
                                        ...
                                        <l-circle  class="layer-circle" v-for="item in circles" :lat-lng="item.center" :radius="item.radius" :color="item.color" :fill-color="item.fillColor" :fill-opacity="item.fillOpacity" :key="item.key" :weight="item.weight">
                                                <l-popup ref="myPopup" >
                                                    <h6 class="text-center">
                                                        <span v-if="item.description" v-html="item.description"></span> <br>
                                                        <em v-if="item.species" v-html="item.species"></em> 
  ...
  export default defineComponent({
            ...
            highlightedItem() 
            {
                if (this.highlightedItem!==null) 
                {
                    this.circles = this.circles.map( (item) => { 
                        if (item.key == this.highlightedItem.sample.id)
                        {
        ...
        methods: {
            ...
            getLastOccurrences() {
                ...
                self.loading=true;
                $('#cover-spin').show(0);
                axios.get(`${API_URL}/api/orthoptera/occurences/last/${self.nrItems}`)  
                    .then(function(response) {
                        if ((response.data) && (response.data.occurrences) && (response.data.occurrences.length>0))
                        {
                            ...
                            self.circles = response.data.occurrences.map( (item) => {
                                return {
                                    ...
                                    href: item.sample.links[0].href,
                                    species: item.taxon.name,
                                    color: "#dc3545",
                                    fillColor: "#dc3545",
                                    fillOpacity: 0.8,
                        ...
                        {
                            self.note = "<h3 style='text-center'>Ups! Entschuldigen Sie bitte, aber aktuell können keine Funde gelesen werden!</h3>"
                        }
                        self.loading=false;
