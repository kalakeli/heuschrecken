            ...
            <div class="row" v-if="( (theMethods) && (theMethods.length>0))">
                <div v-for="item in theMethods" :key="item.methodPKID">
                    <div class="col-12">
                        ...
                        <!--  -->                        
                        <h2 v-if="((item.methodTitle_EN) && (item.methodTitle_EN.length>0))" v-html="item.methodTitle_EN" class="mb-3"></h2>
                        <h2 v-else v-html="item.methodTitle" class="mb-3"></h2>
                        ...
                        <template v-if="((item.methodText_EN) && (item.methodText_EN.length>0))">
                            <div v-html="item.methodText_EN"></div>
                        </template>
                        <template v-if="((!item.methodText_EN) && (item.methodText.length>0))">
...
<script>
  ...
  export default defineComponent({
        ...
        computed: {
            note() {
                if ((this.theMethods) && (this.theMethods.length==1))
                {
                    let item = this.theMethods[0]
                    if ((item.methodTitle_EN == null) || (item.methodTeaser_EN == null) || (item.methodText_EN == null) )
                    {
